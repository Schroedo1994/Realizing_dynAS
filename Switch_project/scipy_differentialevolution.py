...
def differential_evolution(func, bounds, args=(), strategy='best1bin',
                           maxiter = None, popsize=5, tol=1e-12,
...
class DifferentialEvolutionSolver(object):
    ...
    def __init__(self, func, bounds, args=(),
                 strategy='best1bin', maxiter = None, popsize=5,
                 tol=1e-12, mutation=(0.5, 1), recombination=0.7, seed=None,
        ...
        ### Tracking parameters for PLOTTING
    ...
    def solve(self):
        ...
        for i in range(0, self.num_population_members):
            self.a2_x_hist.append(self._scale_parameters(self.population[i]))
            self.f_hist.append(self.func(self._scale_parameters(self.population[i])))
            self.generation_counter.append(self.k)


        # do the optimization.
        #print(f'starting iteration loop, break condition: {self.stop()}')
        while not self.stop():
            # evolve the population by a generation
            #print(f'iteration loop, pop: {self.population}')
            self.k += 1
            try:
                next(self)
            ...
            #print(f'current cond: {self.stop()} evals: {self.fitness_func.evaluations} prec: {self.fitness_func.best_so_far_precision}')


            # should the solver terminate?
            if warning_flag or self.stop():
                break
            ...
            for candidate in range(self.num_population_members):
                ...
                if self._wrapped_constraints:
                    ...
                    if not np.sum(cv) > 0:
                        ...
                        feasible = True
                        self.a2_x_hist.append(parameters)
                        self.generation_counter.append(self.k)
                        energy = self.func(parameters)
                        self.f_hist.append(energy)
                        self._nfev += 1
                else:
                    feasible = True
                    cv = np.atleast_2d([0.])
